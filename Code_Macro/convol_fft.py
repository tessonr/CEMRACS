...
def convol_fft(dx, dy, fftphi, f, Nx, Ny):
    ...
    fftf = np.fft.fft2(np.reshape(f,(Nx,Ny)))
...
def discrete_convol_fft(dx,dy,phi1, phi2, f, g, Nx, Ny):  # calcul d'une convolution
    Xi = convol_fft(dx, dy, phi1, f, Nx, Ny) + convol_fft(dx, dy, phi2, g, Nx, Ny)
    return Xi
