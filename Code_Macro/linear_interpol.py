...
def interp_NS(f,th,dy,Nx,Ny):
    ...
    # change the negative interpolations
    BadI=(fN<0)|(fS<0)
    newslopes=mmd.minmod(th*(f[BadI]-sft.shift_N(f,Nx,Ny)[BadI])/dx,slope[BadI],th*(sft.shift_S(f,Nx,Ny)[BadI]-f[BadI])/dx)
    fN[BadI]=f[BadI]+(dx/2.)*newslopes
    fS[BadI]=f[BadI]-(dx/2.)*newslopes
    return [fN,fS]
